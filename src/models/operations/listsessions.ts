...
export type ListSessionsMetadatum = {
  ...
  guids?: Array<ListSessionsGuid> | undefined;
  /**
...
/** @internal */
export const ListSessionsGuid$inboundSchema: z.ZodType<
  ListSessionsGuid,
  unknown
> = z.object({
  id: types.string(),
...
export function listSessionsGuidFromJSON(
  jsonString: string,
): SafeParseResult<ListSessionsGuid, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => ListSessionsGuid$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'ListSessionsGuid' from JSON`,
  );
}
...
> = z.object({
  MediaContainer: types.optional(
    z.lazy(() => ListSessionsMediaContainer$inboundSchema),
  ),
}).transform((v) => {
  return remap$(v, {
...
> = z.object({
  ...
  Result: z.lazy(() => ListSessionsResponseBody$inboundSchema),
}).transform((v) => {
  return remap$(v, {
    "Headers": "headers",
    "Result": "result",

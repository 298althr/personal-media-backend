 ...
 */
export function epgComputeChannelMap(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ComputeChannelMapRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/livetv/epg/channelmap")();

  const query = encodeFormQuery({
    "device": payload.deviceQueryParameter,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ComputeChannelMapResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([404, "4XX"]),

 ...
 */
export function libraryCreateMarker(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateMarkerRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/metadata/{ids}/marker")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateMarkerResponse$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

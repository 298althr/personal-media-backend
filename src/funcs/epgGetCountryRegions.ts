 ...
 */
export function epgGetCountryRegions(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCountryRegionsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/livetv/epg/countries/{country}/{epgId}/regions")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCountryRegionsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([404, "4XX"]),

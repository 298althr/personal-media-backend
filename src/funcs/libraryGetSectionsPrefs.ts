 ...
 */
export function libraryGetSectionsPrefs(
  client: PlexAPICore,
  request: operations.GetSectionsPrefsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.LibrarySections,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSectionsPrefsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/prefs")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.LibrarySections$inboundSchema),
    M.fail([400, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

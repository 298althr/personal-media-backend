 ...
 */
export function libraryPlaylistsGetPlaylistGenerators(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetPlaylistGeneratorsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/playlists/{playlistId}/generators")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPlaylistGeneratorsResponse$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

 ...
 */
export function contentListMoments(
  client: PlexAPICore,
  request: operations.ListMomentsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.MediaContainerWithArtwork,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListMomentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/{sectionId}/moment")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithArtwork$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

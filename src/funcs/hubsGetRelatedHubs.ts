 ...
 */
export function hubsGetRelatedHubs(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetRelatedHubsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    metadataId: encodeSimple("metadataId", payload.metadataId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/hubs/metadata/{metadataId}/related")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetRelatedHubsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([400, "4XX"]),

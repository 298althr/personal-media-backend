 ...
 * Set a device's channel mapping
 */
export function devicesSetChannelmap(
  client: PlexAPICore,
  request: operations.SetChannelmapRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.MediaContainerWithDevice,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SetChannelmapRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    deviceId: encodeSimple("deviceId", payload.deviceId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/media/grabbers/devices/{deviceId}/channelmap")(
    pathParams,
  );
  ...
  const query = queryJoin(
    encodeDeepObjectQuery({
      ...
      "channelMappingByKey": payload.channelMappingByKey,
    }),
    encodeFormQuery({
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithDevice$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

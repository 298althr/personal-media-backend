 ...
 */
export function contentGetMetadataItem(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetMetadataItemRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/metadata/{ids}")(pathParams);
  ...
  const query = encodeFormQuery({
    ...
    "augmentCount": payload.augmentCount,
    "checkFileAvailability": payload.checkFileAvailability,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetMetadataItemResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail("4XX"),

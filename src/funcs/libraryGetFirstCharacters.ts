 ...
 */
export function libraryGetFirstCharacters(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetFirstCharactersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/{sectionId}/firstCharacters")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetFirstCharactersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

 ...
 */
export function libraryRefreshItemsMetadata(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.RefreshItemsMetadataRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/metadata/{ids}/refresh")(pathParams);

  const query = encodeFormQuery({
    "agent": payload.agent,
    "markUpdated": payload.markUpdated,
  });

 ...
 */
export function libraryGetItemTree(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetItemTreeRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/metadata/{ids}/tree")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithNestedMetadata$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

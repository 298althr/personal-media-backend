 ...
 */
export function downloadQueueGetDownloadQueueItems(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetDownloadQueueItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/downloadQueue/{queueId}/items/{itemId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetDownloadQueueItemsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

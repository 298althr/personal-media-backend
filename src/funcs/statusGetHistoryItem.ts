 ...
 */
export function statusGetHistoryItem(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetHistoryItemRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    historyId: encodeSimple("historyId", payload.historyId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/status/sessions/history/{historyId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetHistoryItemResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([404, "4XX"]),

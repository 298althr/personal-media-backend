 ...
 */
export function libraryListTopUsers(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListTopUsersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/metadata/{ids}/users/top")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListTopUsersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

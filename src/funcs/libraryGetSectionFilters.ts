 ...
 */
export function libraryGetSectionFilters(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSectionFiltersRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/{sectionId}/filters")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetSectionFiltersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

...
export enum GetItemArtworkAcceptEnum {
 ...
 */
export function libraryGetItemArtwork(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetItemArtworkRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    element: encodeSimple("element", payload.element, {
      explode: false,
    ...
    timestamp: encodeSimple("timestamp", payload.timestamp, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/library/metadata/{ids}/{element}/{timestamp}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.stream(200, operations.GetItemArtworkResponse$inboundSchema, {
      ctype: "audio/mpeg3",
    }),
    M.stream(200, operations.GetItemArtworkResponse$inboundSchema, {
      ctype: "image/jpeg",

 ...
 */
export function libraryEditMarker(
  client: PlexAPICore,
  request: operations.EditMarkerRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.PostResponses200,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.EditMarkerRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    marker: encodeSimple("marker", payload.marker, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.PostResponses200$inboundSchema),
    M.fail([400, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

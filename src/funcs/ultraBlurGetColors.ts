 ...
 */
export function ultraBlurGetColors(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetColorsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/services/ultrablur/colors")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetColorsResponse$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail([500, "5XX"]),
  )(response, req);
  if (!result.ok) {

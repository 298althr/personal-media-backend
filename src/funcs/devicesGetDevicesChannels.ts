 ...
 */
export function devicesGetDevicesChannels(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetDevicesChannelsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/media/grabbers/devices/{deviceId}/channels")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetDevicesChannelsResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([404, "4XX"]),

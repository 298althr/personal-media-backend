...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CreateSubscriptionRequest$outboundSchema.parse(value),
  ...
  const query = queryJoin(
    ...
    encodeFormQuery({
      ...
      "targetSectionLocationID": payload.targetSectionLocationID,
      "type": payload.type,
    }),
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateSubscriptionResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail([400, 403, 409, "4XX"]),
    M.fail("5XX"),

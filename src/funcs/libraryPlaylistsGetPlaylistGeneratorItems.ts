 ...
 * Get a playlist generator's items
 */
export function libraryPlaylistsGetPlaylistGeneratorItems(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetPlaylistGeneratorItemsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    generatorId: encodeSimple("generatorId", payload.generatorId, {
      explode: false,
  ...
  const path = pathToFunc("/playlists/{playlistId}/items/{generatorId}/items")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPlaylistGeneratorItemsResponse$inboundSchema),
    M.fail([400, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

 ...
 */
export function downloadQueueGetDownloadQueue(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetDownloadQueueRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/downloadQueue/{queueId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetDownloadQueueResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

 ...
 */
export function hubsUpdateHubVisibility(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.UpdateHubVisibilityRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/hubs/sections/{sectionId}/manage/{identifier}")(
    pathParams,
  );

  const query = encodeFormQuery({
    "promotedToOwnHome": payload.promotedToOwnHome,
    "promotedToRecommended": payload.promotedToRecommended,
    "promotedToSharedHome": payload.promotedToSharedHome,
  });

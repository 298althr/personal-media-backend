 ...
 */
export function epgGetCountriesLineups(
  client: PlexAPICore,
  request: operations.GetCountriesLineupsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.MediaContainerWithLineup,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetCountriesLineupsRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    country: encodeSimple("country", payload.country, {
      explode: false,
      charEncoding: "percent",
    }),
    epgId: encodeSimple("epgId", payload.epgId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/livetv/epg/countries/{country}/{epgId}/lineups")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithLineup$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

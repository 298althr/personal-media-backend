 ...
 */
export function epgGetLineupChannels(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetLineupChannelsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/livetv/epg/lineupchannels")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetLineupChannelsResponse$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

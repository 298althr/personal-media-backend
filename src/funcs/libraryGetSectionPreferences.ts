 ...
 */
export function libraryGetSectionPreferences(
  client: PlexAPICore,
  request: operations.GetSectionPreferencesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    shared.MediaContainerWithSettings,
    | PlexAPIError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetSectionPreferencesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/library/sections/{sectionId}/prefs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, shared.MediaContainerWithSettings$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {

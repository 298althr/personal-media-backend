 ...
 */
export function dvRsSetDVRPreferences(
  client: PlexAPICore,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.SetDVRPreferencesRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    dvrId: encodeSimple("dvrId", payload.dvrId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  const path = pathToFunc("/livetv/dvrs/{dvrId}/prefs")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SetDVRPreferencesResponse$inboundSchema, {
      hdrs: true,
      key: "Result",
    }),
    M.fail("4XX"),
